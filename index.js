...
async function writeArticle(cfg, topic, webLinks, userData, dashboardUrl) {
  ...
  try {
    ...
    let dataNote = '';
    if (userData) {
      // Trim to ~3000 chars so it doesn't dominate the prompt
    ...
    const r = await client.messages.create({
      ...
      messages: [{ role: 'user', content: `Write a blog article (6-8 paragraphs, at least 700 words) about "${topic}" to accompany a data infographic. Rules:
...
- IMPORTANT: the article MUST be at least 700 words. Do not cut it short.${sourceNote}${dataNote}${dashNote}` }]
    });
...
async function runTopic(cfg, entry) {
  ...
  if (cfg.blogId && screenshotUrl) {
    const article = await writeArticle(cfg, entry.topic, entry.webLinks, entry.data || null, htmlUrl);
