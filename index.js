...
// ── GitHub Pages ─────────────────────────────────────────────────────────────
// Outputs go to a separate repo (githubOutputRepo) so pushes don't trigger Railway redeploys.
// Falls back to githubRepo if no output repo is configured.
async function pushToGitHub(cfg, filename, content, isBase64) {
  const outputRepo = cfg.githubOutputRepo || cfg.githubRepo;
  const url = `https://api.github.com/repos/${cfg.githubUser}/${outputRepo}/contents/${filename}`;
  ...
  const pagesUrl = `https://${cfg.githubUser}.github.io/${outputRepo}/${filename}`;
